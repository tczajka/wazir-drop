use std::str::FromStr;
use wazir_drop::{
    constants::MAX_VARIATION_LENGTH, ExtendableVariation, LongVariation, Move, Variation,
};

#[test]
fn test_long_variation() {
    let mut variation = LongVariation::empty();
    variation = variation.add_front(Move::from_str("A@a1").unwrap());
    variation = variation.add_front(Move::from_str("a@a2").unwrap());
